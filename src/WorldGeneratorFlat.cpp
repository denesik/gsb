...
Layering WorldGeneratorFlat2::GetLayering(const DataBase & db, int x, int z) const
{
  ...
  layering += std::make_pair(boost::icl::interval<unsigned short>::right_open(0, land_level), stone);
  layering += std::make_pair(boost::icl::interval<unsigned short>::right_open(land_level, hill_level), dirt);
  layering += std::make_pair(boost::icl::interval<unsigned short>::right_open(hill_level, hill_level + 1), grass);
  layering += std::make_pair(boost::icl::interval<unsigned short>::right_open(hill_level + 1, water_level), water);
  layering += std::make_pair(boost::icl::interval<unsigned short>::right_open(std::max(static_cast<unsigned short>(hill_level + 1), water_level), max), air);
